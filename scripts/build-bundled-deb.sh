...
bundle_source=""
staging_root="${PROJECT_ROOT}/dist/deb/staging"
...
usage() {
    cat <<'EOF'
Usage: scripts/build-bundled-deb.sh [options]

Build a bundled offline-friendly Debian package for Reva Player. The package
installs the application under /opt/revaplayer and includes the runtime
libraries/plugins that are already bundled in the AppImage payload.

Options:
  --build-dir DIR        CMake build directory (default: /tmp/reva-player-build-check)
...
rewrite_desktop_entry() {
    ...
    awk '
        ...
        { print }
        END {
            print "X-Reva-Bundle=offline-deb"
        }
...
Description: Reva Player bundled offline Debian package
 ...
 .
 This package is meant for Debian-based desktops where offline installation
 matters more than strict repository-native dependency resolution.
EOF
}
...
write_control_file "${debian_root}/control" "${version}" "${deb_arch}" "${installed_size}"

rm -f "${output_path}"
dpkg-deb --build --root-owner-group "${package_root}" "${output_path}" >/dev/null

printf '%s bundled Debian package: %s\n' "${APP_DISPLAY_NAME}" "${output_path}"
