...
if options.socket == "" then
    if os_name == "windows" then
        ...
        options.socket = default_runtime_dir().."/revaplayer-thumbfast"
    end
end
...
if options.thumbnail == "" then
    if os_name == "windows" then
        ...
        options.thumbnail = default_runtime_dir().."/revaplayer-thumbfast.out"
    end
end
...
local rename_missing_count = 0
local rename_retry_after = 0

local function remove_output_files()
...
local function move_file(from, to)
    if not mp.utils.file_info(from) then
...
local function check_new_thumb()
    ...
    if now < rename_retry_after then
        return false
    end
    ...
    local moved, reason = move_file(options.thumbnail, tmp)
    if not moved then
        if reason == "missing" then
            rename_missing_count = rename_missing_count + 1
            if rename_missing_count >= 3 then
                rename_retry_after = now + 0.20
            end
        else
            rename_retry_after = now + 0.25
        end
        return false
    end

    rename_missing_count = 0
    rename_retry_after = 0
    local finfo = mp.utils.file_info(tmp)
    if not finfo then return false end
    spawn_waiting = false
    local w, h = real_res(effective_w, effective_h, finfo.size)
    if w then -- only accept valid thumbnails
        local promoted = move_file(tmp, options.thumbnail..".bgra")
